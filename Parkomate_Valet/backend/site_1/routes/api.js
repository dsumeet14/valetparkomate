...
router.post('/car-in', async (req, res) => {
  ...
  try {
    ...
    console.error("Car In error (site1):", err);
...
router.get('/car-requests/:site_no', async (req, res) => {
  ...
  try {
    ...
    const [rows] = await db.query(
      `SELECT car_no, valet_id, status,
              driver_assigned_for_parking, driver_assigned_for_parking_name,
              driver_assigned_for_bringing, driver_assigned_for_bringing_name,
              parking_spot,
...
router.post('/assign-driver', async (req, res) => {
  ...
  try {
    ...
    // Get latest active driver session for name & photo
    const [sessionRows] = await db.query(
      `SELECT driver_name, driver_photo 
FROM \`${login_table}\`
WHERE id=? AND site_no=? AND role="driver" AND last_logout IS NULL 
       ORDER BY last_login DESC LIMIT 1`,
      [driver_id, site_no]
    );
    const driverName = sessionRows.length ? sessionRows[0].driver_name : null;
    ...
    if (['in_request', 'assigned_parking'].includes(st)) {
      ...
      updateQuery = `
        UPDATE \`${car_table}\`
        SET driver_assigned_for_parking=?, driver_assigned_for_parking_name=?,
            timestamp_driver_assigned=NOW(), status="assigned_parking", seen_parking=0
        WHERE car_no=? AND site_no=?`;
      params = [driver_id, driverName, car_no, site_no];
    } else if (['out_request', 'assigned_bringing'].includes(st)) {
      // assign/reassign for bringing
      updateQuery = `
        UPDATE \`${car_table}\`
        SET driver_assigned_for_bringing=?, driver_assigned_for_bringing_name=?,
            timestamp_driver_assigned=NOW(), status="assigned_bringing", seen_bringing=0
        WHERE car_no=? AND site_no=?`;
      params = [driver_id, driverName, car_no, site_no];
    } else {
    ...
    res.json({ message: `Driver ${driverName || driver_id} assigned/reassigned to car ${car_no}.` });
  } catch (err) {
...
/* =========================================================
   MANAGER: DRIVER POOL
========================================================= */
router.get('/driver-pool/:site_no', async (req, res) => {
  const { site_no } = req.params;
  try {
    const { login_table } = await getTablesForSite(site_no);

    const [rows] = await db.query(
      `SELECT id AS driver_id, driver_name, driver_photo,
              on_duty,
              last_login, last_logout
       FROM \`${login_table}\`
       WHERE role="driver" AND site_no=?`,
      [site_no]
    );

    res.json(rows);
  } catch (err) {
    console.error("Driver pool error:", err);
    res.status(500).json({ message: 'Server error', error: err.message });
  }
});
  ...
  try {
    ...
    const driverStats = await Promise.all(
      drivers.map(async (driver) => {
        ...
        return {
          driver_id: driverId,
          total_jobs_today: totalCompletedJobs,
    ...
    await db.query(
      `INSERT INTO \`${dump_table}\` 
       (car_no, valet_id, site_no, phone_number, parking_spot,
        driver_assigned_for_parking, driver_assigned_for_parking_name,
        driver_assigned_for_bringing, driver_assigned_for_bringing_name, status,
      ...
      [
        ...
        carRow.driver_assigned_for_parking_name,
        carRow.driver_assigned_for_bringing,
        carRow.driver_assigned_for_bringing_name,
        'handed_over',
...
router.post('/driver-session/start', async (req, res) => {
  const { driver_id, site_no, driver_name, driver_photo } = req.body;
  try {
    const { login_table } = await getTablesForSite(site_no);

    await db.query(
      `UPDATE \`${login_table}\`
       SET driver_name=?, driver_photo=?, on_duty=1,
           last_login=NOW(), last_logout=NULL
       WHERE id=? AND site_no=? AND role="driver"`,
      [driver_name, driver_photo, driver_id, site_no]
    );

    res.json({ message: 'Driver session started' });
  } catch (err) {
    console.error("Driver session start error:", err);
    res.status(500).json({ message: 'Server error', error: err.message });
  }
});
...
// Driver session end
router.post('/driver-session/end', async (req, res) => {
  const { driver_id, site_no } = req.body;
  try {
    const { login_table } = await getTablesForSite(site_no);

    // First fetch current active session info
    const [rows] = await db.query(
      `SELECT id, site_no, driver_name, driver_photo, last_login
       FROM \`${login_table}\`
       WHERE id=? AND site_no=? AND role="driver" AND on_duty=1`,
      [driver_id, site_no]
    );
    if (!rows.length) {
      return res.status(400).json({ message: "No active session found for driver." });
    }

    const driver = rows[0];
    const logoutTime = new Date();

    // Dump into driver_sessions
    await db.query(
      `INSERT INTO driver_sessions (driver_id, site_no, driver_name, driver_photo, login_time, logout_time)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [
        driver.id,
        driver.site_no,
    ...
    // Reset login_1 entry → inactive & clear temporary info
    await db.query(
      `UPDATE \`${login_table}\`
       SET on_duty=0,
           driver_name=NULL,
    ...
    res.json({ message: 'Driver session ended and dumped' });
  } catch (err) {
    console.error("Driver session end error:", err);
    res.status(500).json({ message: 'Server error', error: err.message });
  }
});
  ...
  try {
    ...
    const [rows] = await db.query(
      `SELECT car_no, valet_id, status, parking_spot,
       ...
       FROM \`${car_table}\`
       WHERE site_no=? AND (
         (driver_assigned_for_parking=? AND status="assigned_parking")
...
router.post('/driver-seen', async (req, res) => {
  const { car_no, site_no, driver_id, type } = req.body;
  try {
    const { car_table } = await getTablesForSite(site_no);
